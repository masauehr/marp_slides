...
- **Markdownのリンク記法（`[text](url)`）はハイパーリンクとして再構築されない**（2026-08-20、`2026-09-18-vibe-coding.md` の参考文献リンクで確認）。HTML上はaタグとして正しく出力されるが、LibreOffice再構築後の`.pptx`ではリンク文字列がリンク色（青）の**プレーンテキスト**になるだけで、クリック可能なハイパーリンク（`<a:hlinkClick>`）にはならず、URL自体もテキストとして残らない。PowerPoint上でリンク先を確認する手段が失われる
  - 対処法: リンクテキストとは別に、URLをそのまま本文に併記する（例: `URL: https://example.com` を独立した行として追加）。本文の一部としてChromiumがレンダリングするため、LibreOffice再構築後も文字列としてそのまま残る。通常の`--pptx`（画像焼き込み）・`--pdf`は影響を受けない
...
| 2026-08-01 | `2026-09-18-vibe-coding.md` の「vibe codingとは」説明を改訂。「ノリ駆動開発」という訳語だけでは自虐的な印象になるため、提唱者アンドレイ・カルパシー氏が原文で "not too bad for throwaway weekend projects" と明言していた文脈を追加し、個人開発・プロトタイピング向きであり商用アプリ開発とは切り分けて語られた概念であることを明示 |
| 2026-08-01 | `hero-gradient`/`hero-geo`/`hero-side`/`hero-diagonal` の4クラスを `themes/ml-forecast-multicolor.css`・`themes/ml-forecast-vivid.css` にも移植し、`vibe-coding.md` の3配色版PPTX（`-multicolor.pptx`/`-vivid.pptx`）を作成。その過程で、`ml-forecast-multicolor.css` のh1が使っていたグラデーション文字（`background-clip: text`）も `--pptx-editable`（LibreOffice再構築）で正しく再現されず、スライドごとに「黒文字のみ」「文字の後ろに色帯が乗り文字は黒のまま」とバラバラに崩れる不具合を発見。グラデーション文字はeditable PPTX運用と根本的に相性が悪いため、h1を単色（アクセントカラー1色）＋下線装飾に変更して解決（詳細は「編集可能な PPTX を作る」節の注意点を参照） |
...
| 2026-08-19 | `.md`内容の修正（「vibe codingという言葉の由来」等の文言更新）を受けて再変換した際、`--html`フラグを付け忘れ、2026-08-01に一度修正済みだったスライド10・13のflexbox横並び画像が再び縦積み・画面外はみ出しの不具合を再発させてしまった。`--html`を付けて再生成し復旧（PDF・PPTX×3配色すべて）。あわせて正式運用の`--pptx-editable`で再生成し、既知の不具合（表紙・まとめスライドの`hero-gradient`背景がLibreOffice変換で崩れる）に対する画像焼き込みハイブリッド差し替え（スライド1・22）も3配色分すべてに再適用。**教訓**: このファイルは`--html`必須が2度目の見落としとなったため、再変換前に必ず本セクションの一覧表で必須オプションを確認すること |
| 2026-08-19 | スライド21「vibe coding X GitHub 私の開発スタイル」のポンチ絵を全面刷新。作業フォルダを中心に①自然言語でコード生成②毎回マニュアル化③GitHubへ自動push・定時実行で日々確認④アプリ高度化・バグ修正、が双方向に循環するハブ型の図（`slides/Claude開発サイクル_ポンチ絵.svg`）を新規作成し、`images/dev_cycle.svg`に上書きコピーしてスライドへ反映（画像幅は`w:420`→最終的に`w:540`へ拡大）。検討過程でリング型・タイムライン型の別案2種（`slides/Claude開発サイクル_案A_リング型.svg`／`_案C_タイムライン型.svg`）も作成し保存。スライド本文の箇条書き・引用も新しい図の内容（フォルダ蓄積・自動push・スマホ確認・アイデア/バグ発見のフィードバック）に合わせて更新し、引用ブロックを右カラムのリスト直下に配置換え。さらに1枚「この開発サイクルのポイント」（なぜプロンプト不要か／GitHubで見るもの／バイブコーディングとしての強み）を新規追加し、全23枚構成に。開発スタイルの詳細は`slides/2026-08-19-claude-code-dev-style.md`にも別途まとめた。**制約の再確認**: 今回は新しいSVG（`dev_cycle.svg`）が`--pptx-editable`変換でも正しく画像として埋め込まれ、2026-07-28に見つかった「SVGが空白になる」不具合は再発しなかった（SVGの内容・複雑さによって挙動が変わる可能性があるため、SVGを差し替えるたびに要確認） |
| 2026-08-20 | `2026-09-18-vibe-coding.md` の参考文献リンク（Qiita記事）がPPTXに反映されていない不具合を修正。原因は、Markdownのリンク記法がChromiumレンダリングではaタグとして出力されるものの、`--pptx-editable`のLibreOffice再構築でクリック不可のプレーンテキストになり、URL自体もテキストとして残らないこと（新しい制約として「編集可能な PPTX を作る」節に追記）。対処としてリンクとは別にURLを本文に併記する行を追加し、3配色版すべてを再変換。再変換の副作用として、表紙・まとめの`hero-gradient`背景スライドが既知の不具合（LibreOfficeによる背景崩れ）で再度崩れたため、`python-pptx`によるハイブリッド画像差し替えを3配色分すべてに再適用 |
